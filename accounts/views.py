...
def cadastro(request):
    ...
    if not nome or not sobrenome or not email or not usuario \
        ...
        messages.error(request, 'Nenhum campo pode está vazio.')
        return render(request, 'accounts/cadastro.html')
    ...
    if len(senha) < 6:
        messages.error(request, 'Senha precisa ser com pelo menos 6 caracteres.')
        return render(request, 'accounts/cadastro.html')
    
    if len(usuario) < 6:
        messages.error(request, 'Usuario precisa ser com pelo menos 6 caracteres.')
        return render(request, 'accounts/cadastro.html')
